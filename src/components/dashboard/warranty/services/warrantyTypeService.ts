...
export async function createWarrantyType (data: {
  ...
  try {
    ...
    if (data.duration === undefined || data.duration < 0) {
      throw new Error('Duration must be a positive number or zero for lifetime warranties');
    }
...
export async function deleteWarrantyType (id: string) {
  try {
    ...
    if (warrantiesCount > 0) {
      throw new Error(`Cannot delete this warranty type because it is being used by ${warrantiesCount} warranties`);
    }
