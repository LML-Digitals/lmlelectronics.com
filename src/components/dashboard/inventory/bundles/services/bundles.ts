...
export async function createBundleVariation(
  data: z.infer<typeof CreateBundleVariationSchema>
) {
  try {
...
export async function addBundleComponents(
  data: z.infer<typeof AddBundleComponentsSchema>
) {
  try {
...
export async function getBundleStockAllLocations(bundleItemId: string) {
  try {
    ...
    for (const [locationId, locationData] of locationMap) {
      for (const component of components) {
        const stockLevel = component.componentVariation.stockLevels.find(
  ...
  try {
    ...
    for (const component of components) {
      ...
      if (availableStock < requiredStock) {
        throw new Error(
          `Insufficient stock for component ${component.componentVariation.name}`
        );
      }
    }
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (const component of components) {
        ...
        const stockLevels = [...component.componentVariation.stockLevels].sort(
        ...
        if (remainingToDeduct > 0) {
          throw new Error(
            `Unexpected error: not enough stock deducted for component ${component.componentVariation.name}`
          );
        }
      }
    });
...
export async function getBundles(locationId?: number) {
  try {
    ...
    const bundlesWithStock = await Promise.all(
