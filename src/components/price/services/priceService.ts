...
export async function repairToPriceItem(
  repair: RepairOptionProps
): Promise<PriceItem> {
...
export async function searchPrices(
  ...
  if (type === "all" || type === "repair") {
    try {
      ...
      const repairItemPromises = repairs.map((repair) => 
        repairToPriceItem(repair as RepairOptionProps)
      );
  ...
  if (type === "all" || type === "product") {
    try {
      ...
      const productItemPromises = products.map((product) =>
